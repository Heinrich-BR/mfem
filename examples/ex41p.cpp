...
void Ricci2D::updateBLFCoefs() {
  ...
  _thermal_exp.reset(new TransformedCoefficient(
      _T_gfcoef.get(), _phi_gfcoef.get(), [this](real_t T, real_t phi) {
...
void Ricci2D::updateLFCoefs() {
  ...
  _Fn_no_source.reset(new TransformedCoefficient(
      ...
      [this](real_t n, real_t exp_term) { return (n / 24.) * exp_term; }));

  _Fn.reset(new TransformedCoefficient(
      _Fn_no_source.get(), _Sn.get(),
      [this](real_t Fn_no_source, real_t Sn) { return Fn_no_source - Sn; }));

  _FT_no_source.reset(new TransformedCoefficient(
      _T_gfcoef.get(), _thermal_exp.get(), [this](real_t T, real_t exp_term) {
        return (T / 36.) * (1.71 * exp_term - 0.71);
      }));

  _FT.reset(new TransformedCoefficient(
      _FT_no_source.get(), _Sn.get(),
      [this](real_t FT_no_source, real_t Sn) { return FT_no_source - Sn; }));
...
void Ricci2D::formMKB() {
  ...
  for (int i=0; i<3; ++i)
  {
      ParBilinearForm *m = new ParBilinearForm(_h1_fes.get());
      ParBilinearForm *k = new ParBilinearForm(_h1_fes.get());
      ParLinearForm *b = new ParLinearForm(_h1_fes.get());

      // Form K matrix

      SumIntegrator *k_int = new SumIntegrator;
      k_int->AddIntegrator(new ConvectionIntegrator(*_vd, -_Binv)); // Poisson bracket first term
      k_int->AddIntegrator(new MassIntegrator(*_div_vd_scaled)); // Poisson bracket second term
      k_int->AddIntegrator(new DiffusionIntegrator(*_SUW_matcoef)); // SUW term
      ScaleIntegrator *k_dt_int = new ScaleIntegrator(k_int, _dt); // K matrix multiplied by dt
      ...
      if (i == 0)
          m_k_dt_int->AddIntegrator(new MassIntegrator(*_DnFn)); // Gateaux derivative term, already scaled by dt
      else if (i == 1)
          m_k_dt_int->AddIntegrator(new MassIntegrator(*_DTFT)); // Gateaux derivative term, already scaled by dt
      ...
      if (i == 0)
         b->AddDomainIntegrator(new DomainLFIntegrator(*_Fn));
      else if (i == 1)
         b->AddDomainIntegrator(new DomainLFIntegrator(*_FT));
      else if (i == 2)
         b->AddDomainIntegrator(new DomainLFIntegrator(*_Fw));
  ...
  m_01->Assemble();
  m_01->Finalize();
  _h_blocks(0, 1) = m_01->ParallelAssemble();
  ...
  m_21->Assemble();
  m_21->Finalize();
  _h_blocks(2, 1) = m_21->ParallelAssemble();

  _h_blocks(0,2) = nullptr;
  _h_blocks(1,2) = nullptr;
  _h_blocks(1,0) = nullptr;
  _h_blocks(2,0) = nullptr;
  ...
  for (int i=0;i<3; ++i)
  {
   for (int j=0;j<3;++j)
   {
      std::cout << "_h_blocks_" << i << j << " = " << _h_blocks(i,j) << std::endl;
   }
  }
